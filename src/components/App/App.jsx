    ...
    <div>
      ...
      {toDoList.map((taskData, dataIndex) => {
        return (
          <div
            key={dataIndex}
            className={taskData.Status ? 'Completed' : 'Uncompleted'}>
            <h3>
              {' '}
              Task {dataIndex + 1}: {taskData.Task}
            </h3>
            <div>
              <p>
                {' '}
            ...
            <div>
              ...
              <i>"{taskData.Status ? 'Completed' : 'Uncompleted'}"</i>
            </div>
            <p>
              <input
                type="checkbox"
                onClick={(event) => handleTaskStatus(taskData.id)}></input>{' '}
            </p>
